...
```{r}
q_star_agg <- demand_agg(p=.10) # q_star_agg = 816,884 kwH
p_star_before_tax <- .10 #10 cents is p star
```

```{r}
# find slope for supply curve
mpc_slope = p_star_before_tax/q_star_agg # slope of supply curve = 1.22e-07
...
mec_per_kwh <- .13 # <- marginal external cost in dollars
```
...
agg_consumer_surplus <- CS_agg(p=.10) # = 1.2e7
# convert to dollars
agg_cs_dollars <- agg_consumer_surplus/100 # = $119,991
```
...
PS_status_quo <- PS(p = .10) # = 408,442 cents producer surplus under status quo
#convert to dollars
PS_sq_dollars <- PS_status_quo/100 # = $4,084 
```

```{r}
# find environmental damage costs under status quo

env_cost <- .02 # $0.01966 per kwh of env. damage
total_env_cost_sq <- env_cost * q_star_agg # = $16,338 of env damage under status quo q_star of 816,884 kwh
```

```{r}
# find consumer surplus for low income vs. high income
low_cs_sq <- CS(p = .10, model = low.income.lm) # 2,457,278 with the status quo, convert to dollars
low_cs_sq_dollars <- low_cs_sq/100 # = $24,573

high_cs_sq <- CS(p = .10, model = high.income.lm) # 9541781 cents with status quo, convert to dollars
high_cs_sq_dollars <- high_cs_sq/100 # = $95,418

# High income has a much higher consumer surplus than low income. High income has a surplus of $70,845 higher than low income.
