...
P_star_demand <- tax(51)+10 #= 11.966
...
PS_tax <- (0.5* Q_demand_agg_tax(p = 10, scc = 51) * P_star_51)/100 # $25,081 turning it into dollars
...
```{r}
tax_rev_75 <-(Q_demand_agg_tax(p = 10, scc = 75)) * ((env_cost+10) - Supply_fun(q = Q_tax_51))/100 #change q = to new q

```


```{r}
high_welfare <- ((CS(p=10, model = high.income.lm)) +
                   high_income_proportion * 
                   (Q_demand_agg_tax(p = 10, scc = 51))* #this and down is tax rev
                   ((env_cost + 10) -
                      Supply_fun(q = Q_demand_agg_tax(p=10, scc=51))))/100
                                                                                    # = $40,967
```
